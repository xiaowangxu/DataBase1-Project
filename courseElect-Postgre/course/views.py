...
def get_Appliction_Paged(request):
    if (request.method == 'POST'):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT c.id as id, c.name as name, c.credit as credit, c.description as description, c.cid as cid, c.depart as depart, c.capacity, t.name as tname, t.id as tuid, t.tid as tid, c.term as term FROM course_course c JOIN teacher_teacher t on c.keys_tid_id = t.id WHERE C.accept is NULL ORDER BY term DESC, cid")
...
def has_Appliction(request):
    if (request.method == 'POST'):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT count(c.id) as count FROM course_course c JOIN teacher_teacher t on c.keys_tid_id = t.id WHERE c.accept is NULL")
...
def get_Course_Paged(request):
    if (request.method == 'POST'):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT c.id as id, c.name as name, c.description as description, c.capacity, CC.count as currentcount, c.credit as credit, c.cid as cid, c.depart as depart, t.name as tname, t.id as tuid, t.tid as tid, c.term as term FROM course_course c JOIN teacher_teacher t on c.keys_tid_id = t.id JOIN course_capacity as CC ON CC.id = C.id WHERE C.accept is TRUE ORDER BY c.term DESC, cid")
            qs = list(dictfetchall(cursor))
            p = Paginator(qs, 10)
            pageidx = min(request.params["page"], p.num_pages)
            retStr = list(p.page(pageidx).object_list)
...
def get_Course_by_Student(request):
    if (request.method == 'POST'):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT C.id as id, C.name as name, C.cid as cid, C.credit as credit, C.capacity as capacity, CC.count as currentcount, C.description as description, C.depart as depart, T.name as tname, T.id as tuid, T.tid as tid FROM course_course C JOIN teacher_teacher T on C.keys_tid_id = T.id JOIN course_capacity as CC ON CC.id = C.id WHERE C.depart = %s and C.id not in (SELECT cid_id FROM election_election WHERE sid_id = %s and grade is not NULL) and C.term in (SELECT current FROM \"currentTerm_currentterm\") and C.accept is True", [s.depart, s.id])
                ret = list(dictfetchall(cursor))
            ret2 = []
...
def get_Course_by_Teacher(request):
    if (request.method == 'POST'):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT C.id as id, C.name as name, C.credit as credit, C.cid as cid, C.depart as depart, C.term as term, C.capacity as capacity, C.grade_percent, T.id as tuid, T.tid as tid, T.name as tname, A.avg FROM course_course C JOIN teacher_teacher T on T.id = C.keys_tid_id LEFT JOIN ( SELECT avg(grade) avg, cid_id cid FROM election_election GROUP BY cid_id ) A on A.cid = C.id WHERE T.id = %s and C.accept and C.term = %s", [t.id, request.params['term']])
                ret = list(dictfetchall(cursor))
                return JsonResponse({'list': ret})
    ...
    if (request.method == 'POST'):
        ...
        try:
            ...
            try:
                ...
                target.capacity = request.params['capacity']
                try:
                    target.save()
                except:
                    return JsonResponse({'state': 'failed', 'data': '无法修改'})
