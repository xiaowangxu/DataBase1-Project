...
def get_GPAs(request):
    if (request.method == 'POST'):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT term term, avg(grade) avg, sum(jd * credit)/sum(credit) as gpa FROM (SELECT C.term, E.grade, gpa(grade) as jd, C.credit FROM election_election E JOIN course_course C on E.cid_id = C.id WHERE E.sid_id = %s and C.term not in (SELECT current from \"currentTerm_currentterm\")) as score GROUP BY term ORDER BY term DESC", [request.params['id']])
            ret = list(dictfetchall(cursor))
            return JsonResponse({'list': ret})
